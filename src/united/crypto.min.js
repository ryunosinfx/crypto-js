...
class Base {
	...
	static mixIn = (base, properties) => {
		...
		if (properties.hasOwnProperty('toString')) base.toString = properties.toString;
		return base;
	};
	...
	clone() {
		...
		for (const k in this) {
			const value = this[k];
			newOne[k] = value === undefined ? undefined : JSON.parse(JSON.stringify(value));
		}
		return newOne;
	}
}
...
class WordArray extends Base {
	...
	concat(wordArray) {
		const thisWords = this.words,
			thatWordsOrigin = wordArray.words,
			thatWords = WordArray.thatWords;
		thatWords.splice(0, thatWords.length);
		...
		else for (let j = 0; j < thatSigBytes; j += 4) thisWords[(thisSigBytes + j) >>> 2] = thatWords[j >>> 2];
...
class X64WordArray extends Base {
	...
	toX32() {
		const x64Words = this.words,
			x64WordsLength = x64Words.length,
			x32Words = [];
...
class Base64 {
	...
	static stringify = wordArray => {
		...
		for (let i = 0; i < sigBytes; i += 3) {
			const byte1 = (words[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff,
				byte2 = (words[(i + 1) >>> 2] >>> (24 - ((i + 1) % 4) * 8)) & 0xff,
				byte3 = (words[(i + 2) >>> 2] >>> (24 - ((i + 2) % 4) * 8)) & 0xff,
	...
	static parse = base64Str => {
		const map = Base64._map,
			reverseMap = Base64._reverseMap,
			paddingChar = map.charAt(64),
			paddingIndex = paddingChar ? base64Str.indexOf(paddingChar) : -1,
			base64StrLength = paddingIndex !== -1 ? paddingIndex : base64Str.length;
...
class Base64url {
	...
	static _reverseMap = [];
	static _safe_reverseMap = [];
	static stringify = (wordArray, urlSafe = true) => {
		const words = wordArray.words,
			sigBytes = wordArray.sigBytes,
			map = urlSafe ? Base64url._safe_map : Base64url._map;
		wordArray.clamp();

		const b64c = [];
		for (let i = 0; i < sigBytes; i += 3) {
			const b1 = (words[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff,
				b2 = (words[(i + 1) >>> 2] >>> (24 - ((i + 1) % 4) * 8)) & 0xff,
				b3 = (words[(i + 2) >>> 2] >>> (24 - ((i + 2) % 4) * 8)) & 0xff,
				triplet = (b1 << 16) | (b2 << 8) | b3;
			for (let j = 0; j < 4 && i + j * 0.75 < sigBytes; j++)
				b64c.push(map.charAt((triplet >>> (6 * (3 - j))) & 0x3f));
		}
		const pC = map.charAt(64);
		if (pC) while (b64c.length % 4) b64c.push(pC);
		return b64c.join('');
	};
	static parse = (base64Str, urlSafe = true) => {
		const map = urlSafe ? Base64url._safe_map : Base64url._map,
			reverseMap = urlSafe ? Base64url._safe_reverseMap : Base64url._reverseMap,
			paddingChar = map.charAt(64),
			paddingIndex = paddingChar ? base64Str.indexOf(paddingChar) : -1,
			base64StrLength = paddingIndex !== -1 ? paddingIndex : base64Str.length;
	...
	static init = () => {
		const map = Base64url._map,
			smap = Base64url._safe_map,
			ml = map.length,
			sl = smap.length,
			rm = Base64url._reverseMap,
			sm = Base64url._safe_reverseMap;
...
class Latin1 {
	...
	static stringify = wordArray => {
		const words = wordArray.words,
			sigBytes = wordArray.sigBytes,
			latin1Chars = Latin1.latin1Chars;
		latin1Chars.splice(0, latin1Chars.length);
...
class Hex {
	...
	static stringify = wordArray => {
		...
		const hexChars = Hex.hexChars;
		hexChars.splice(0, hexChars.length);
		for (let i = 0; i < sigBytes; i++) {
			const bite = (words[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff;
...
class BufferedBlockAlgorithm extends Base {
	...
	_process(isDoFlush) {
		...
		if (nWordsReady) {
			for (let offset = 0; offset < nWordsReady; offset += blockSize) this._doProcessBlock(dataWords, offset);
...
class HMAC extends Base {
	constructor(hasherClass, keyOrigin) {
		super();
		const hasherInited = new hasherClass();
		this._hasher = hasherInited;
		const key = typeof keyOrigin === 'string' ? Utf8.parse(keyOrigin) : keyOrigin,
			hasherBlockSize = hasherInited.blockSize,
			hasherBlockSizeBytes = hasherBlockSize * 4,
			keyFinalized = key.sigBytes > hasherBlockSizeBytes ? hasherInited.finalize(key) : key;
		keyFinalized.clamp();
		const oKey = keyFinalized.clone(),
			iKey = keyFinalized.clone(),
...
class Hasher extends BufferedBlockAlgorithm {
	...
	finalize(messageUpdate) {
		...
		const finalResult = this._doFinalize();
		this.reset();
		return finalResult;
	}
...
class Cipher extends BufferedBlockAlgorithm {
	...
	constructor(isEncryption, key, cfg = {}) {
		...
		this.cfg = Base.mixIn(this.cfg, Cipher.conf);
...
class StreamCipher extends Cipher {
	...
	constructor(isEncryption, key, cfg) {
		...
		this.cfg = Base.mixIn(this.cfg, cfg);
		this.blockSize = 1;
		const a = StreamCipher.addConf;
		a.keySize = this.keySize;
		a.ivSize = this.ivSize;
		a.blockSize = this.blockSize;
		this.cfg = Base.mixIn(this.cfg, a);
	}
	_doFinalize() {
...
class BlockCipher extends Cipher {
	...
	reset() {
		...
		const cfg = this.cfg,
			iv = cfg.iv,
			mode = cfg.mode,
			isRestBlockMode = this.isEncryption,
			modeCreator = isRestBlockMode ? mode.createEncryptor : mode.createDecryptor;
		if (!isRestBlockMode) this._minBufferSize = 1; /* if (this._xformMode == this._DEC_XFORM_MODE) */
...
class CipherParams extends Base {
	...
	constructor(cipherParams = CipherParams.defaultValue) {
		super();
		for (const k in cipherParams) this[k] = cipherParams[k];
...
class OpenSSLFormatter {
	static stringify = cipherParams => {
		const ciphertext = cipherParams.ciphertext,
			...
			wordArray = salt ? new WordArray([0x53616c74, 0x65645f5f]).concat(salt).concat(ciphertext) : ciphertext;
		return wordArray.toString(Base64);
	};
	static parse = openSSLStr => {
		let salt;
		const ct = Base64.parse(openSSLStr),
			ctw = ct.words;
		if (ctw[0] == 0x53616c74 && ctw[1] == 0x65645f5f) {
			salt = new WordArray(ctw.slice(2, 4));
			ctw.splice(0, 4);
			ct.sigBytes -= 16;
		}
		return new CipherParams({ ciphertext: ct, salt });
	};
}
class SerializableCipher extends Base {
	...
	encrypt(cipherClass, message, key, cfg) {
		const cfgExtended = Base.mixInAsNew(this.cfg, cfg);
		const encryptor = new cipherClass(true, key, cfgExtended);
	...
	decrypt(cipherClass, ciphertext, key, cfg) {
		const cfgExtended = Base.mixInAsNew(this.cfg, cfg);
		const ciphertextParsed = this._parse(ciphertext, cfgExtended.format);
		return new cipherClass(false, key, cfgExtended).finalize(ciphertextParsed.ciphertext);
	}
...
class OpenSSLKdf {
	...
	static execute = (password, keySize, ivSize, salt, hasher) => {
		const saltForUse = salt ? salt : WordArray.random(64 / 8);
		const key = hasher
		...
		return new CipherParams({ key, iv, salt: saltForUse });
	};
}
class PasswordBasedCipher extends SerializableCipher {
	...
	encrypt(cipherClass, message, password, cfg) {
		const cfgCurrent = Base.mixIn({}, this.cfg);
		const cfgExtended = Base.mixIn(cfgCurrent, cfg);
		...
		cfgExtended.iv = derivedParams.iv;
		const ciphertext = super.encrypt(cipherClass, message, derivedParams.key, cfgExtended);
		Base.mixIn(ciphertext, derivedParams);
		this.ciphertext = ciphertext;
		return ciphertext;
	}
	decrypt(cipher, ciphertext, password, cfg) {
		...
		cfgExtended.iv = derivedParams.iv;
		return super.decrypt(cipher, ciphertextParsed, derivedParams.key, cfgExtended);
	}
}
class Encryptor extends BlockCipherMode {
	constructor(cipher, iv) {
		super(cipher, iv);
		this._prevBlock = iv ? iv.slice(0) : [];
	}
...
class CBC extends BlockCipherMode {
	...
	static xorBlock(self, words, offset, blockSize) {
		...
		if (iv) self._iv = undefined;
		for (let i = 0; i < blockSize; i++) words[offset + i] ^= block[i];
	}
}
...
let xj = 0,
	xi = 0;
for (let i = 0; i < 256; i++) {
	const sxi = xi ^ (xi << 1) ^ (xi << 2) ^ (xi << 3) ^ (xi << 4);
	...
	const x4 = d[x2];
	const x8 = d[x4];
	const s = (d[sx] * 0x101) ^ (sx * 0x1010100);
	SUB_MIX_0[xj] = (s << 24) | (s >>> 8);
	SUB_MIX_1[xj] = (s << 16) | (s >>> 16);
	SUB_MIX_2[xj] = (s << 8) | (s >>> 24);
	SUB_MIX_3[xj] = s;
	const t = (x8 * 0x1010101) ^ (x4 * 0x10001) ^ (x2 * 0x101) ^ (xj * 0x1010100);
...
const generateKeystreamAndEncrypt = (self, words, offset, blockSize, cipher) => {
	const iv = self._iv;
	const keystream = iv ? iv.slice(0) : self._prevBlock;
	if (iv) self._iv = undefined;
	cipher.encryptBlock(keystream, 0);
	for (let i = 0; i < blockSize; i++) words[offset + i] ^= keystream[i];
};
...
class EncryptorCTRGladman extends CTRGladman {
	...
	processBlock(words, offset) {
		...
		const blockSize = cipher.blockSize;
		const iv = this._iv;
		const counter = iv ? (this._counter = iv.slice(0)) : this._counter;
		if (iv) this._iv = undefined;
		incCounter(counter);
...
class EncryptorCTR extends CTR {
	...
	processBlock(words, offset) {
		...
		const blockSize = cipher.blockSize;
		const iv = this._iv;
		const counter = iv ? iv.slice(0) : this._counter;
...
class EncryptorOFB extends OFB {
	...
	processBlock(words, offset) {
		...
		const blockSize = cipher.blockSize;
		const iv = this._iv;
		const keystream = iv ? iv.slice(0) : this._keystream;
...
class Iso97971 {
	...
	static pad = (data, blockSize) => {
		data.concat(new WordArray(Iso97971.initArray, 1));
	...
	compute(password, salt) {
		...
		while (derivedKeyWords.length < keySize) {
			...
			for (let i = 1; i < iterations; i++) {
				...
				for (let j = 0; j < blockWordsLength; j++) blockWords[j] ^= intermediateWords[j];
			}
...
class RC4Drop extends RC4 {
	...
	_doReset() {
		super._doReset();
		for (let i = this.cfg.drop; i > 0; i--) this.generateKeystreamWord();
	}
}
...
for (let x = 0; x < 5; x++) for (let y = 0; y < 5; y++) PI_INDEXES[x + 5 * y] = y + ((2 * x + 3 * y) % 5) * 5;
let LFSR = 0x01;
for (let i = 0; i < 24; i++) {
	...
	for (let j = 0; j < 7; j++) {
		...
		if (LFSR & 0x80) LFSR = (LFSR << 1) ^ 0x71;
		else LFSR <<= 1;
	}
	ROUND_CONSTANTS[i] = new X64Word(roundConstantMsw, roundConstantLsw);
}
for (let i = 0; i < 25; i++) U[i] = new X64Word();
	...
	_doProcessBlock(M, offset) {
		...
		for (let round = 0; round < 24; round++) {
			...
			for (let laneIndex = 1; laneIndex < 25; laneIndex++) {
				...
				const TPiLane = U[PI_INDEXES[laneIndex]];
				TPiLane.high = tMsw;
				TPiLane.low = tLsw;
			}
			const T0 = U[0];
			...
			for (let x = 0; x < 5; x++)
				for (let y = 0; y < 5; y++) {
					...
					const lane = state[laneIndex];
					const TLane = U[laneIndex];
					const Tx1Lane = U[((x + 1) % 5) + 5 * y];
					const Tx2Lane = U[((x + 2) % 5) + 5 * y];
...
const x64w = X64Word;
const J = [
	new x64w(0x428a2f98, 0xd728ae22),
	new x64w(0x71374491, 0x23ef65cd),
	new x64w(0xb5c0fbcf, 0xec4d3b2f),
	new x64w(0xe9b5dba5, 0x8189dbbc),
	new x64w(0x3956c25b, 0xf348b538),
	new x64w(0x59f111f1, 0xb605d019),
	new x64w(0x923f82a4, 0xaf194f9b),
	new x64w(0xab1c5ed5, 0xda6d8118),
	new x64w(0xd807aa98, 0xa3030242),
	new x64w(0x12835b01, 0x45706fbe),
	new x64w(0x243185be, 0x4ee4b28c),
	new x64w(0x550c7dc3, 0xd5ffb4e2),
	new x64w(0x72be5d74, 0xf27b896f),
	new x64w(0x80deb1fe, 0x3b1696b1),
	new x64w(0x9bdc06a7, 0x25c71235),
	new x64w(0xc19bf174, 0xcf692694),
	new x64w(0xe49b69c1, 0x9ef14ad2),
	new x64w(0xefbe4786, 0x384f25e3),
	new x64w(0x0fc19dc6, 0x8b8cd5b5),
	new x64w(0x240ca1cc, 0x77ac9c65),
	new x64w(0x2de92c6f, 0x592b0275),
	new x64w(0x4a7484aa, 0x6ea6e483),
	new x64w(0x5cb0a9dc, 0xbd41fbd4),
	new x64w(0x76f988da, 0x831153b5),
	new x64w(0x983e5152, 0xee66dfab),
	new x64w(0xa831c66d, 0x2db43210),
	new x64w(0xb00327c8, 0x98fb213f),
	new x64w(0xbf597fc7, 0xbeef0ee4),
	new x64w(0xc6e00bf3, 0x3da88fc2),
	new x64w(0xd5a79147, 0x930aa725),
	new x64w(0x06ca6351, 0xe003826f),
	new x64w(0x14292967, 0x0a0e6e70),
	new x64w(0x27b70a85, 0x46d22ffc),
	new x64w(0x2e1b2138, 0x5c26c926),
	new x64w(0x4d2c6dfc, 0x5ac42aed),
	new x64w(0x53380d13, 0x9d95b3df),
	new x64w(0x650a7354, 0x8baf63de),
	new x64w(0x766a0abb, 0x3c77b2a8),
	new x64w(0x81c2c92e, 0x47edaee6),
	new x64w(0x92722c85, 0x1482353b),
	new x64w(0xa2bfe8a1, 0x4cf10364),
	new x64w(0xa81a664b, 0xbc423001),
	new x64w(0xc24b8b70, 0xd0f89791),
	new x64w(0xc76c51a3, 0x0654be30),
	new x64w(0xd192e819, 0xd6ef5218),
	new x64w(0xd6990624, 0x5565a910),
	new x64w(0xf40e3585, 0x5771202a),
	new x64w(0x106aa070, 0x32bbd1b8),
	new x64w(0x19a4c116, 0xb8d2d0c8),
	new x64w(0x1e376c08, 0x5141ab53),
	new x64w(0x2748774c, 0xdf8eeb99),
	new x64w(0x34b0bcb5, 0xe19b48a8),
	new x64w(0x391c0cb3, 0xc5c95a63),
	new x64w(0x4ed8aa4a, 0xe3418acb),
	new x64w(0x5b9cca4f, 0x7763e373),
	new x64w(0x682e6ff3, 0xd6b2b8a3),
	new x64w(0x748f82ee, 0x5defb2fc),
	new x64w(0x78a5636f, 0x43172f60),
	new x64w(0x84c87814, 0xa1f0ab72),
	new x64w(0x8cc70208, 0x1a6439ec),
	new x64w(0x90befffa, 0x23631e28),
	new x64w(0xa4506ceb, 0xde82bde9),
	new x64w(0xbef9a3f7, 0xb2c67915),
	new x64w(0xc67178f2, 0xe372532b),
	new x64w(0xca273ece, 0xea26619c),
	new x64w(0xd186b8c7, 0x21c0c207),
	new x64w(0xeada7dd6, 0xcde0eb1e),
	new x64w(0xf57d4f7f, 0xee6ed178),
	new x64w(0x06f067aa, 0x72176fba),
	new x64w(0x0a637dc5, 0xa2c898a6),
	new x64w(0x113f9804, 0xbef90dae),
	new x64w(0x1b710b35, 0x131c471b),
	new x64w(0x28db77f5, 0x23047d84),
	new x64w(0x32caab7b, 0x40c72493),
	new x64w(0x3c9ebe0a, 0x15c9bebc),
	new x64w(0x431d67c4, 0x9c100d4c),
	new x64w(0x4cc5d4be, 0xcb3e42b6),
	new x64w(0x597f299c, 0xfc657e2a),
	new x64w(0x5fcb6fab, 0x3ad6faec),
	new x64w(0x6c44198c, 0x4a475817),
];
const L = [];
for (let i = 0; i < 80; i++) L[i] = new x64w();
class SHA512 extends Hasher {
	...
	_doReset() {
		this._hash = new X64WordArray([
			new x64w(0x6a09e667, 0xf3bcc908),
			new x64w(0xbb67ae85, 0x84caa73b),
			new x64w(0x3c6ef372, 0xfe94f82b),
			new x64w(0xa54ff53a, 0x5f1d36f1),
			new x64w(0x510e527f, 0xade682d1),
			new x64w(0x9b05688c, 0x2b3e6c1f),
			new x64w(0x1f83d9ab, 0xfb41bd6b),
			new x64w(0x5be0cd19, 0x137e2179),
		]);
	}
	_doProcessBlock(M, offset) {
		const H = this._hash.words;
...
class SHA384 extends SHA512 {
	_doReset() {
		this._hash = new X64WordArray([
			new x64w(0xcbbb9d5d, 0xc1059ed8),
			new x64w(0x629a292a, 0x367cd507),
			new x64w(0x9159015a, 0x3070dd17),
			new x64w(0x152fecd8, 0xf70e5939),
			new x64w(0x67332667, 0xffc00b31),
			new x64w(0x8eb44a87, 0x68581511),
			new x64w(0xdb0c2e0d, 0x64f98fa7),
			new x64w(0x47b5481d, 0xbefa4fa4),
		]);
	}
	_doFinalize() {
		const hash = super._doFinalize();
