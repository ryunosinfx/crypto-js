...
export class UnitTestZeroPadding {
	static C = C;
	...
	static run() {
		const describe = o.describe,
			it = o.it;
		describe('pad-zeropadding-test', function () {
			describe('ZeroPadding', function () {
				it('testPad', () => {
					const data = new C.lib.WordArray([0xdddddd00], 3);
					C.pad.ZeroPadding.pad(data, 2);
					assert.equal(new C.lib.WordArray([0xdddddd00, 0x00000000]).toString(), data.toString());
				});
				...
				it('testUnpad', () => {
					const data = new C.lib.WordArray([0xdddddd00, 0x00000000]);
